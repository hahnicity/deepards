...
from deepards.models.protopnet2d.helpers import makedir, find_high_activation_crop
from deepards.models.protopnet2d.receptive_field import compute_rf_prototype
...
class Pusher(object):
    ...
    def push_orig(self, dataloader, model, epoch_number):
        model.eval()
        self.log('\tpush')
        ...
        for push_iter, (idx, search_batch_input, _, search_y) in enumerate(dataloader):
            '''
            start_index_of_search keeps track of the index of the image
            assigned to serve as prototype
            '''
            search_batch_input = search_batch_input.float()
            start_index_of_search_batch = push_iter * search_batch_size

            self.update_prototypes_on_batch(dataloader,
                                            model,
                                            search_batch_input,
    ...
    def push_protobank(self, dataloader, model, epoch_number):
        model.eval()
        self.log('\tpush')
        ...
        model.protobank_tensor.data.copy_(torch.tensor(prototype_update, dtype=torch.float32).cuda())
        # prototype_network_parallel.cuda()
        end = time.time()
    ...
    def generic_update_search(self,
        ...
        with torch.no_grad():
            ...
            conv_features, distances = model.protobank_distances(search_batch_input)
        ...
        for j in range(n_prototypes):
            ...
            for i in range(self.bank_size):
                ...
                if which_less_than.any():
                    ...
                    # XXX I forgot to set all row vals to np.inf for the selected row
    ...
    def update_prototypes_on_batch(self,
        ...
        with torch.no_grad():
            ...
            conv_features, distances = model.prototype_distances(search_batch)
