...
class CNNLSTMNetwork(nn.Module):
    def __init__(self, breath_block, metadata_features):
        ...
        self.lstm = nn.LSTM(breath_block.inplanes+metadata_features, self.lstm_hidden_units, num_layers=self.lstm_layers, batch_first=True)
    ...
    def forward(self, x, metadata):
        ...
        if not torch.any(torch.isnan(metadata)):
