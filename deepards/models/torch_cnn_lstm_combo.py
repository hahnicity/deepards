...
class CNNLSTMDoubleLinearNetwork(nn.Module):
    def __init__(self, breath_block, metadata_features, bm_to_linear, lstm_hidden_units, n_sub_batches):
        ...
        self.linear_intermed = nn.Linear((self.lstm_hidden_units+metadata_features)*n_sub_batches, self.lstm_hidden_units+metadata_features)
        self.linear_final = nn.Linear(self.lstm_hidden_units+metadata_features, 2)
    ...
    def forward(self, x, metadata):
        ...
        x, (hx, cx) = self.lstm(outputs)
        if self.bm_to_linear:
            x = torch.cat([x, metadata], dim=-1)
        x = self.linear_intermed(x.contiguous().view(x.shape[0], -1))
        x = self.linear_final(x)
        return x
