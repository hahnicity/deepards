...
class RetinaNetMain(nn.Module):

    def __init__(self, backbone):
        super().__init__()
        # make backbone first, then make the retinanet. Currently this class is
        # only configured for densenet. However if we use other backbones then
        # we can modify this code.
        extra_blocks = LastLevelP6P7(128, 128)
        returned_layers = [2, 3, 4]
        return_layers = {f'denseblock{k}': str(v) for v, k in enumerate(returned_layers)}
        in_channels_stage2 = backbone.n_out_filters // 8
        in_channels_list = [128] * len(returned_layers)
        out_channels = 128
        # I think that currently if we are using the FPN as described in the retinanet
        # paper then we should just use the LastLevelP6P7 regardless of whether or
        # not it makes all the sense to do it
