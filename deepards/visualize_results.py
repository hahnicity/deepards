...
parser.add_argument('-b', '--base-network', default='resnet18')
...
all_vals = None
glob_search = 'results/test_auc*_{network}_base{base_network}_e{epochs}_nb{num_sub_batches}_lc{loss_calc}_rip{initial_planes}_lvp{lstm_vote_percentage}_rfptmax_optim{optim}*'
glob_search = glob_search.format(**args.__dict__)

for i, f in enumerate(sorted(glob(glob_search))):
